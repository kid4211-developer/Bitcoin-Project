...
import * as MainChart from '../components/StyledComponents/CoinMainChartComponent'
...
import withOHLCData from './withContainer/withOHLCData'
import withThemeData from './withContainer/withThemeData'
import withSelectedOption from './withContainer/withSelectedOption'
import withLoadingData from './withContainer/withLoadingData'
import isEqual from 'react-fast-compare'
import { startAddMoreCandleData } from '../action/CoinDataAction'
...
// Coin Main Chart Component
const CoinMainChart = ({ data: initialData, height, width, ratio, selectedTimeType, theme, isCandleLoading }) => {
    if (height > 500) {
        height = 500
    }
    const dispatch = useDispatch()

    const dateTimeFormat = selectedTimeType === 'days' || selectedTimeType === 'weeks' ? '%y-%m-%d' : '%y-%m-%d %H:%M'
    ...
    return (
        <MainChart.ChartContainer>
            <MainChart.HiddenH3>캔들 차트</MainChart.HiddenH3>
            {isCandleLoading ? (
                <Loading />
            ) : (
                <ChartCanvas
...
export default withOHLCData()(
    ...
    })(withDeviceRatio()(withSelectedOption()(withLoadingData()(withThemeData()(React.memo(CoinMainChart, isEqual))))))
)
