...
const withSelectedCoinPrice = () => (OriginalComponent) => (props) => {
    ...
    const selectedCoinData = useSelector((state) => state.Coin.candle.data[selectedMarket])
    ...
    // 24시간 가격 변화율, 변화량
    const changeRate24Hour = Math.round(selectedCoinData.changeRate24Hour * 10000) / 100
